# Laser Harp — Wiring Reference (Beam 0 bring-up)

Current as of the DFPlayer Mini audio design. Use this to reverify every
connection on the bench. Matches the code in `atmega32-fw/src/main.c` and
`esp32-fw/src/main.cpp`.
...
- Resistors: 10kΩ ×5, 220Ω ×1 (only these two values used, per what's on hand)
- 1× LDR (beam 0 sensor)
- 1× LED (beam 0 indicator)
- USBasp programmer
...
## Beam 0 sensor → ATmega32 pin 40 (PA0 / ADC0)

| # | Connection |
|---|---|
| C1 | 5V-A → LDR leg 1 |
| C2 | LDR leg 2 → node "C" |
| C3 | node C → ATmega32 **pin 40 (PA0)** |
| C4 | node C → 10kΩ resistor → GND |

## Beam 0 indicator LED → ATmega32 pin 1 (PB0)

| # | Connection |
|---|---|
| D1 | ATmega32 pin 1 (PB0) → 220Ω resistor → LED anode (long leg) |
| D2 | LED cathode (short leg) → GND |
...
everything above. Format FAT32, create a folder named exactly `MP3` at
the root, and put `0001_C4.mp3` … `0007_B4.mp3` inside that folder
(already prepped in the repo at `sd-card/MP3/`).

## Bring-up order

1. Build Section B + C + D + E only. Flash the ATmega32. Confirm the LED
   on PB0 responds to blocking beam 0 — nothing else connected yet.
2. Build Section G on the ESP32 side (DFPlayer + speaker), with the SD
   card loaded. Flash the ESP32. Confirm "DFPlayer Mini ready" over
   serial with nothing on Serial2 yet.
3. Add Section F (the UART link) last. Block beam 0 and confirm
   "Beam 0 blocked -> playMp3Folder(1)" in the serial monitor, and the
   note actually playing through the speaker.
